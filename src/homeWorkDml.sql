...
SELECT * FROM people;
--2. Обновить электронную почту конкретного человека (например, человека
--с id=1)
UPDATE people SET first_name = 'Rouz' where id =1;
--3. --Добавьте новую запись в таблицу
insert into people(id, first_name, last_name, email, gender, age, language)
values (51,'vdnjsk','fdsv','vhjd@hvjfd','Male',14,'Russian');
--4. Удалить человека из таблицы (например, человека с id=10)
delete from people where id=51;
...
select sum(people.age) from people;
--9. Получить самого молодого человека(людей)
select min(people.age) from people;
--10. Сгруппировать людей по полу и рассчитать средний возраст для каждого
--пола
select people.gender, avg(age) from people group by gender;
--11. Вывести имя в год рождение людей которые говорят на английском
--языке
select people.first_name, people.age from people where language='English';
--12. Посчитать сколько человек, говорящих на казахском(Kazakh)
select count(people.language) from people where language='Kazakh';
--13. Вывести фамилии, почту и айди всех женщин
select id, concat(people.last_name,' ',people.first_name), email  from people where gender = 'Male';
--14. Вывести имя и фамилие вместе переименовав как full_name(concat as),
--пол и год рождение всех мужчин родившихся после 2000 года
select concat(people.first_name,' ', people.last_name) as full_name,age from people where gender='Male';
...
select people.first_name, people.email from people order by email;
--17. Вывести 20 записей пропустив первые 5 записи
select * from people limit 20 offset 5;
--18. Подсчитать количество людей для каждого языка
select people.language, count(*) as count_people from people group by language;
--19. Найти средний возраст для каждого языка, где количество людей больше 2
select people.language, round(avg(people.age)) from people group by people.language  having count(*)>2;
...
select people.language, count(*) AS count from people where people.gender='Female' group by language order by count desc limit 1;
--22. Обновить язык для людей, чей возраст между 3 и 10 годами, на 'French'
update people set language= 'French' where age between 13 and 20;
--23. Отобразите всех мужчин говорящих на Tamil, Kazakh, Nepali и Russian
select * from people where people.language in('Tamil','Kazakh','Nepali','Russian')  and gender='Male';
--24. Сгруппируте всех по языку на котором они говорят
select language, COUNT(*) AS people_count from people group by language;
--25. Отобразите записи фамилии которых содержат только 7 символов и
--отсортируйте их по айди
select  people.id,people.last_name  from people where length(last_name)=7 order by id;
...
--28. Отобразите только те записи из таблицы которым есть 18 и которых
--почта заканчивается на .com или .uk
...
select * from people where age not between 13 and 20;
--30. Отобразите уникальные языки
select people.language from people group by language;
--31. Отобразите записи где в имени или в фамилии есть буква ‘a’ и год
--рождение с 1969 по 2005 год
select * from people where first_name like '%a%' or last_name like '%a%' and age between 20 and 40;
--32. Отобразите айди, имя, почту и возврст каждого человека и отсортируйте
...
delete from people where age<16;
